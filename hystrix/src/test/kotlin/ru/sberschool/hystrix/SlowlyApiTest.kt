...
class SlowlyApiTest {
    ...
    private val pokeApiClient: SlowlyApi = HystrixFeign.builder()
        .client(ApacheHttpClient())
        .decoder(JacksonDecoder())
        // для удобства тестирования задаем таймауты на 1 секунду
        .options(Request.Options(1, TimeUnit.SECONDS, 1, TimeUnit.SECONDS, true))
    ...
    @Test
    fun `Api client should return ditto pokemon`() {
        assertEquals("ditto", pokeApiClient.getPokemon("ditto").name)
    }

    @Test
    fun `Api client should return bulbasaur pokemon`() {
        assertEquals("bulbasaur", pokeApiClient.getPokemon("bulbasaur").name)
    }

    @Test
    fun `getPokemon() should return predefined data`() {
        // given
        ...
        // expect
        assertEquals("ditto", client.getPokemon("ditto").name)
    }
}
